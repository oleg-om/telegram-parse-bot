...
config();
const puppeteerScript = (bot, chatId, env, stickers) => {
  const {
    WEBSITE,
    USER_LOGIN,
    USER_PASSWORD,
  ...
  (async () => {
    ...
    try {
      ...
      const getCategories = async () => {
        ...
        console.log("WANTED_SERVICE", WANTED_SERVICE);

        // services
        const services = await page.evaluate(
          (WANTED_SERVICE, SERVICES_CLASS) => {
            return Array.from(
              document.querySelectorAll(`[class="${SERVICES_CLASS}"]`),
              (element) => element?.textContent === WANTED_SERVICE
            )?.filter((it) => it);
          },
          WANTED_SERVICE,
          SERVICES_CLASS
        ...
        if (servicesLength && servicesLength > 0) {
          ...
          bot.sendMessage(
            chatId,
            `На данный момент талонов: ${servicesLength}`
          );
        }
...
// puppeteerScript({sendMessage:()=>{}}, null, process.env, true);
